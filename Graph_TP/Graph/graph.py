...
#Inicializa el grafo (dirigido o no) y define el criterio de orden por valor.
class Graph(List):

    #Crea un vértice con su valor, lista de aristas, posibles valores adicionales y marca de visitado.
    class __nodeVertex:
        ...
        #Devuelve una representación en texto del vértice (su valor).
        def __str__(self):
            return self.value
    
    class __nodeEdge:

       #Crea una arista con su vértice destino, peso y valores adicionales opcionales.
        def __init__(self, value: Any, weight: Any, other_values: Optional[Any] = None):
            self.value = value
            self.weight = weight
            self.other_values = other_values # no esta en uso aun
        
        #Devuelve una representación en texto de la arista, mostrando destino y peso.
        def __str__(self):
    ...
    #Devuelve el valor de un vértice para usarlo como criterio de ordenamiento.
    def _order_by_value(item):
        return item.value

    #Devuelve el peso de una arista para usarlo como criterio de ordenamiento.
    def _order_by_weight(item):
        return item.weight
    
    #Muestra todos los vértices y sus aristas en pantalla.
    def show(
        self
    ) -> None:
        for vertex in self:
    ...
    #Conecta dos vértices mediante una arista con un peso dado (agrega en ambos sentidos si el grafo es no dirigido).
   ...
   #Elimina la arista que une dos vértices (y su inversa si el grafo es no dirigido).
    def delete_edge(
    self,
    origin,
    destination,
   ...
   #Elimina un vértice del grafo y todas las aristas que lo conectan.
    def delete_vertex(
        self,
        value,
    ...
    #Marca todos los vértices como no visitados (útil para recorridos).
    def mark_as_unvisited(self) -> None:
        for vertex in self:
            vertex.visited = False

    #Determina si existe un camino entre dos vértices usando una búsqueda en profundidad recursiva.
    ...
    #Realiza un recorrido en profundidad (DFS) mostrando los vértices visitados.
    ...
    #Realiza un recorrido en amplitud (BFS) mostrando los vértices visitados.    
    ...
    #Aplica el algoritmo de Dijkstra para hallar las rutas más cortas desde un vértice origen hacia los demás.
    ...
    #Aplica el algoritmo de Kruskal para construir el árbol de expansión mínima del grafo.
